//--------------------------------------------
// Input/output module for G4Na61 analyis
// Author: Paweł Staszel
//
// Some new changes: Mateusz Bajda (start in 03_05_22)
//--------------------------------------------
#if !defined NA61_Na61AlVdTrackingHTModule
...
// ClassImp(Na61AlVdTrackingHTModule);

//____________________________________________________________________
Na61AlVdTrackingHTModule::Na61AlVdTrackingHTModule() : Na61AlVdTrackingModule() {
...
//____________________________________________________________________
Na61AlVdTrackingHTModule::Na61AlVdTrackingHTModule(const char* name, const char* title) : Na61AlVdTrackingModule(name, title) {
  // Named Constructor
  SetState(kSetup);
...
//____________________________________________________________________
void Na61AlVdTrackingHTModule::DefineHistograms() {
  if (GetState() != kInit) {
  ...
  fGr_axay_vdtrack->SetMarkerStyle(21);
...
//____________________________________________________________________
void Na61AlVdTrackingHTModule::Init() {
...
//____________________________________________________________________
void Na61AlVdTrackingHTModule::Begin() {
...
//____________________________________________________________________
void Na61AlVdTrackingHTModule::Event(UEventNode* inNodeJ, UEventNode* inNodeS, UEventNode* outNode) {
  // Per event method
...
void Na61AlVdTrackingHTModule::Add4HitTracks(UEventNode* inNodeJ, UEventNode* inNodeS, UEventNode* outNode) {
...
void Na61AlVdTrackingHTModule::SetTrackHits(UVdTrack* track, UEventNode* in) {
  int ind1 = track->GetHitIndexOnStation(0);
  int ind2 = track->GetHitIndexOnStation(1);
  int ind3 = track->GetHitIndexOnStation(2);
  int ind4 = track->GetHitIndexOnStation(3);
  ...
  if (ind1 != -1) hit1 = (USensorHit*)(in->GetDataTable(Form("Hits %s", fAlSensorNames[tind1].Data())))->At(ind1);
  if (ind2 != -1) hit2 = (USensorHit*)(in->GetDataTable(Form("Hits %s", fAlSensorNames[tind2].Data())))->At(ind2);
  if (ind3 != -1) hit3 = (USensorHit*)(in->GetDataTable(Form("Hits %s", fAlSensorNames[tind3].Data())))->At(ind3);
  if (ind4 != -1) hit4 = (USensorHit*)(in->GetDataTable(Form("Hits %s", fAlSensorNames[tind4].Data())))->At(ind4);

  track->SetVdsHitIDs(hit1, hit2, hit3, hit4);
}

//_____________________________________________________________
bool Na61AlVdTrackingHTModule::CheckTrackMatching2(UVdTrack* track1, UVdTrack* track2) {
...
void Na61AlVdTrackingHTModule::FillCombinatorialCurvature(UEventNode* in) {
...
//_________________________________________________________________
void Na61AlVdTrackingHTModule::RecoWithSimultaneousHT(UEventNode* inNodeJ, UEventNode* inNodeS, UEventNode* outNode) {
  // algorithm:
  ...
  if (((UVdEvent*)outNode)->GetPrimaryVertexStatus()) {
    ...
    std::cout<<"(Na61AlVdTrackingHTModule line 419) EventVertexX = ((UVdEvent*)outNode)->GetPrimaryVertexX()= " << EventVertexX << endl;
    std::cout<<"(Na61AlVdTrackingHTModule line 419) EventVertexY = ((UVdEvent*)outNode)->GetPrimaryVertexY() = " << EventVertexY << endl;
    EventVertexZ = ((UVdEvent*)outNode)->GetPrimaryVertexZ() + fVzOffset * 0.001;
    EventVertexZorg = ((UVdEvent*)outNode)->GetPrimaryVertexZ();
    std::cout<<"(Na61AlVdTrackingHTModule line 419) EventVertexZ = ((UVdEvent*)outNode)->GetPrimaryVertexZ() + fVzOffset*0.001 = " 
    <<((UVdEvent*)outNode)->GetPrimaryVertexZ() + fVzOffset*0.001 << std::endl;
    std::cout<<"(Na61AlVdTrackingHTModule line 421) EventVertexZorg = ((UVdEvent*)outNode)->GetPrimaryVertexZ()  = " 
    <<((UVdEvent*)outNode)->GetPrimaryVertexZ() << std::endl;
  } else {
  ...
  std::cout<<"(Na61AlVdTrackingHTModule line 434) offx, sigx = " << offx << ", "<< sigx <<std::endl;
  std::cout<<"(Na61AlVdTrackingHTModule line 435) offy, sigy = " << offy << ", "<< sigy <<std::endl;
  ...
  cout<<"(Na61AlVdTrackingHTModule line 440) dd="<<dd<<" "<<offx<<" "<< sigx<<" "<<offy<<" "<<sigy<<endl;
  if (dd > 5 * 5) {
    cout<<"(Na61AlVdTrackingHTModule line 442) RecoWithSimultaneousHT: event not accepted:"<<endl;
  ...
  */
  ...
  if( !((EventVertexX>-3.0) && (EventVertexX<5.0)) ){
    cout<<"(Na61AlVdTrackingHTModule line 456) RecoWithSimultaneousHT: event not accepted (bad X):"<<endl;
  ...
  if( !((EventVertexY>-4.0) && (EventVertexY<3.0)) ){
    cout<<"(Na61AlVdTrackingHTModule line 464) RecoWithSimultaneousHT: event not accepted (bad Y):"<<endl;
    cout<<"Primary vertex x = "<<EventVertexX<<endl;
    cout<<"Primary vertex y = "<<EventVertexY<<endl;
    cout<<"Primary vertex z = "<<EventVertexZ<<endl;
    return;
  ...
  */
  ...
  std::cout<<" (Na61AlVdTrackingHTModule line 472) RecoWithSimultaneousHT: PrimVertexZ="<< EventVertexZ <<endl;

  // UDataTable* table = new UDataTable("Vd HT Tracks");
  UDataTable* table = new UDataTable(fOutTableName.Data());
  table->SetOwner();

  TObjArray* hitsVds1 = GetHits("Al1", inNodeJ, inNodeS);
  TObjArray* hitsVds2 = GetHits("Al2", inNodeJ, inNodeS);
  TObjArray* hitsVds3 = GetHits("Al3", inNodeJ, inNodeS);
  TObjArray* hitsVds4 = GetHits("Al4", inNodeJ, inNodeS);
  ...
  /////// using Hough transform
   ...
   std::cout<<"---------------------------------> start HT points="<< fhHSaxay->GetEntries()<<" integral="<<fhHSaxay->Integral()<<endl;
  ...
  for (int itab = 0; itab < 4; itab++) {
    ...
    for (int i = 0; i < vdsHits->GetEntries(); i++) {
      ...
      std::cout << "Numer stacji: " << itab + 1  << std::endl;
      std::cout << "Położenie hitu:  x, y, z: " << hit->GetX() <<", " << hit->GetY() << ", "
 << hit->GetZ() << endl;
      std::cout << "Położenie hitu z przesunięciem: x, y, z: " << hit->GetX() - EventVertexX <<", " << hit->GetY() - EventVertexY << ", "
 << hit->GetZ() - EventVertexZ << endl;

      double ax = (x - bx) / z;
      double ay = (y - by) / z;

      std::cout << "Współczynnik ax, ay: " << ax <<", " << ay << endl;
      ...
      //std::cout<<"(Na61AlVdTrackingHTModule line 556) binwidth matrix = "<< fhHSaxay->GetBinWidth(200) <<endl;
      // if((0.122<ax && ax<0.128) && (0.183<ay && ay<0.189)){
      std::cout<<"indx="<<indx<<"  "<<"indy="<<indy<<" ax="<<ax<<" ay="<<ay<<" z="<<z<<endl;
      //}
      ...
      std::cout<<" ii = "<< ii <<" indx="<<indx<<"  indy="<<indy<<endl;
      if (ii != -1) {
        hitInfo = (UHitInfo*)fHitInfoArray->At(ii);
        std::cout<<"hitInfo="<<hitInfo<<endl;
      } else {
      ...
      if (hitInfo->GetEntries() > 98) {
         std::cout << "RecoWithSimultaneousHT: Too many Entries: " << hitInfo->GetEntries() << endl;
        break;
      }
      std::cout << "Informacje o hicie: i, itab " << i << ", " << itab << endl;
  ...
  /*
  This is definition of table (it's situated above)
    // UDataTable* table = new UDataTable("Vd HT Tracks");
  UDataTable* table = new UDataTable(fOutTableName.Data());
  table->SetOwner();
  */
  ...
  for (int i = 0; i < table->GetEntries(); i++) {
    ...
    if (!fProduction) {
      ...
      fGr_axay_vdtrack->SetPoint(i, ax_vdreco, ay_vdreco); //?? Tu jest jakiś proble
    }
...
TObjArray* Na61AlVdTrackingHTModule::GetHits(TString vdsStr, UEventNode* inJ, UEventNode* inS) {
  int stationID = -1;
  if (vdsStr.Contains("Al1")) stationID = 1;
  if (vdsStr.Contains("Al2")) stationID = 2;
  if (vdsStr.Contains("Al3")) stationID = 3;
  if (vdsStr.Contains("Al4")) stationID = 4;

  TObjArray* hits = new TObjArray();
  hits->Clear();

  for (int it = 0; it < 34; it++) {
    if (!fAlSensorNames[it].Contains(vdsStr.Data())) continue;

    cout<<fAlSensorNames[it].Data()<<"  (LINE 635 Na61AlVdTRackingHTModule.cc) "<<vdsStr.Data()<<endl;
    UDataTable* hitstabJ = inJ->GetDataTable(Form("Hits %s", fAlSensorNames[it].Data()));
    ...
    UDataTable* hitstabS = inS->GetDataTable(Form("Hits %s", fAlSensorNames[it].Data()));
...
//_____________________________________________________________________
void Na61AlVdTrackingHTModule::AnaClustersAndCreateTracksNew(UDataTable* table, TObjArray** tabArr) {
...
void Na61AlVdTrackingHTModule::ExtractTracksFromCluster(TObjArray& hitsVds1, TObjArray& hitsVds2, TObjArray& hitsVds3, TObjArray& hitsVds4, UDataTable* table) {
    ...
    for (int j = 0; j < loopIterVds2; j++) {
      ...
      for (int k = 0; k < loopIterVds3; k++) {
        ...
        for (int l = 0; l < loopIterVds4; l++) {
          ...
          //Na61AlVdTrackingModule::FitLine_w2(hits, sig, ax, ay, bx, by, chi2x, chi2y, N, zmin); //(have to be checked -- 05.05.22)
          FitLine_w2(hits, sig, ax, ay, bx, by, chi2x, chi2y, N, zmin);
          cout << "FitLine_w2 (nie) działa" << endl;
          ...
          if (N == 3) {
            ...
            cout << " (Na61AlVdTrackingHTModule linijka 825) N == 3 działa " << endl;
            fhDevX[id]->Fill(devx);
            fhDevY[id]->Fill(devy);
          }
          ...
          if (N == 4) {
            ...
            cout << " (Na61AlVdTrackingHTModule linijka 833) N == 4 działa " << endl;
...
//__________________________________________________________________________________________
void Na61AlVdTrackingHTModule::CalculateDeviations(USensorHit* hit1, USensorHit* hit2, USensorHit* hit3, USensorHit* hit4, double& devx, double& devy) {
  double x[3];
  double y[3];
  double z[3];
...
//__________________________________________________________________________________________
void Na61AlVdTrackingHTModule::CalculateResDeviations(USensorHit** hits, double* sig, double* resx, double* resy) {
  double ay;
  double by;
  //double N;
  //double zmin;

  //FitLineY_w2(hits, sig, ay, by, N, zmin); (have to be checked --- 05.05.22)
...
//_____________________________________________________________
void Na61AlVdTrackingHTModule::CheckTrackMatching(UVdTrack* track1, UVdTrack* track2) {
...
void Na61AlVdTrackingHTModule::FillHSaxayWithClusters() {
...
//_____________________________________________________________________
void Na61AlVdTrackingHTModule::MakeClustersNew() {
  ...
  cout<<"(Na61AlVdTrackingHTModule line 1079) HTModule, 1089 line, N = "<<N<<endl;
  ...
  for (int ind = 0; ind < N; ind++) {
    ...
    cout<< "Na61AlVdTrackingHTModule::MakeClustersNew() --- line 1107" << endl;
    ...
    cout<< "Na61AlVdTrackingHTModule::MakeClustersNew() after using CheckRing(i, j) --- line 1118" << endl;
...
//_____________________________________________________________________
void Na61AlVdTrackingHTModule::CheckRing(int i, int j) {
  for (int ik = i - 1; ik < i + 2; ik++) {
    for (int jk = j - 1; jk < j + 2; jk++) {
      ...
      cout << "CheckRing_4_for " << ik + 1 << " and " <<   jk + 1 << endl;
...
//_____________________________________________________________
void Na61AlVdTrackingHTModule::End() {
  // Run-level finalisation
  SetState(kEnd);
}

//____________________________________________________________________
void Na61AlVdTrackingHTModule::Finish() {
  // Job-level finalisation
  SetState(kFinish);
}

//____________________________________________________________________
void Na61AlVdTrackingHTModule::Print(Option_t* option) const {
  ...
  if (opt.Contains("d")) cout << endl << "  Original author: Paweł Staszel" << endl << "  Last Modifications: " << endl << " Mateusz Bajda $" << endl << "    $Date: 2022/05/3$" << endl << "    $Revision: 1.0 $ " << endl << endl << "-------------------------------------------------" << endl;
}
